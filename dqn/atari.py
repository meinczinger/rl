...
device = "cpu"
# num_gpus = torch.backends.mps.is_available()
...
def epsilon_greedy(state, env, net, epsilon=0.0):
    if np.random.random() < epsilon:
        ...
        # print(state.dtype)
        state = torch.tensor(
            np.array([state]), device=torch.device(device)
        )  # .to(device)

        # state = torch.Tensor(state.shape)
        # state = torch.from_numpy(np.array([state]), device=torch.device("mps"))
