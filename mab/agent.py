...
class ActionSelector(abc.ABC):
    ...
    def select_action(
        self, q_values: list[float], arm_count: list[int] = [], step: int = 0
    ) -> int:
        raise NotImplementedError
...
class UpperConfidenceBoundActionSelector:
    ...
    def select_action(
        self, q_values: list[float], arm_count: list[int], step: int
    ) -> int:
...
ACTION_SELECTOR_TYPE = {
    ActionSelectorEnum.EPSILON_GREEDY: EpsilonGreedyActionSelector,
...
class Agent:
    ...
    def agent_step(self) -> tuple:
        ...
        action = self.action_selector.select_action(
            self.q_values, self.arm_counts, self.step
        )
